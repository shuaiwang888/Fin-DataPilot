...
displayName: 金融数据查询
description: >-
  金融数据查询工具。用于获取标的相关的宏观、财务、行情、事件、交易、个人账户、自选股、个股看点、
  股票概念、技术形态、交易日开盘等数据，覆盖股票、美股、港股、基金、指数、宏观、可转债、期货等领域；
...
本 skill 是**金融数据查询的统一入口**，通过调用同花顺问财 OpenAPI 的 `query2data` 端点，用自然语言获取股票、美股、港股、基金、指数、宏观、可转债、期货等领域的数据。用户未明确标的类型时，默认按**股票**处理。
...
- **交易与形态**：交易数据、技术形态、交易日开盘查询
- **标的画像**：个股看点、股票概念、主营业务与主题归属
...
接收自然语言请求，识别**标的（股票/基金/指数/板块/期货/债券）**、**指标（行情/财务/估值/事件/交易）**、**时间范围**。若问题没有明确标的类型，按**股票**处理。

### 步骤 2: Query 改写

按以下三种输入情况构造查询：

1. **单条件或多条件筛选标的**：`query` 直接使用用户原始问句，所有条件必须同时出现在同一个 `financial-query` 输入中，禁止拆散、删减或自行修改条件。
   - 用户问句：`6月4日个股热度大于100万，6月4日大单买入金额/成交额大于40%`
   - 工具输入：`6月4日个股热度大于100万，6月4日大单买入金额/成交额大于40%`
2. **诊股指标查询**：指标不超过 5 个时尽量一次查询；指标超过 5 个时按相近数据族拆成多个独立 `financial-query` 调用，可并行取数，再由回答层合并。
3. **明确标的 + 指标查询**：查询对应标的的相关数据，支持连续时段、固定时段和指定时间点；输入必须包含具体指标以及明确的时间点或时间范围。

非筛选类问句允许做最小改写，**保持原意不变**：
...
**拆解规则：**
- 条件筛选：无论条件多少，都必须保留在同一个 query 中，不得拆分条件
- 诊股指标：超过 5 个指标时拆成多个独立 query
- 主体、指标或标的过多：在不破坏筛选条件整体性的前提下，按独立取数目标适当拆分

**输入限制：**
- 不支持需要复杂逻辑推断后才能形成的查询
- 不支持天气等非金融指标
- 不支持用抽象事件指代时间，例如“发布重大公告后”；应先用搜索 Skill 找到具体事件日期，再用明确日期查询
- `query` 中不得出现 `资金面`；应改为用户明确要求的具体资金指标，如主力资金净流入、北向资金或融资融券
- 时间必须是具体、明确的时间点或范围；没有时间要求且用户查询最新值时可以省略时间
- 受上下文限制，查询结果只展示部分数据；不能把“展示部分”解释为“市场中只有这些数据”
...
`datas` 为空时，不得自行删除、放宽或修改用户的筛选条件。可使用完全相同的 query 做一次技术性重试，并把 `X-Claw-Call-Type` 改为 `retry`；仍为空时，如实说明未查询到匹配数据。只有用户明确同意放宽条件后，才可以用新条件再次查询。
...
| "中证 500 指数当前点位" | ✅ | — |
| "比亚迪上个月销量" | ✅ | — |
| "6月4日个股热度大于100万，且大单买入金额/成交额大于40%" | ✅，全部条件放在同一个 query | — |
...
- 问的是**"符合一个或多个条件的标的 / 排名 / TopN / 选股"** → 本 skill；全部筛选条件必须放在同一个 query
- 问的是**诊股且指标超过 5 个** → 拆成多个独立本 skill 调用
