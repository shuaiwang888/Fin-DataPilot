...
6. **financial-query 的三类输入规则**：
   - 单条件或多条件筛选：必须把用户原始问句中的全部条件放在同一个 `args.query`，禁止拆散、删减、放宽或自行改写条件；用户没要求 TopN 时不要擅自增加。
...
10. **不要**重复同一步的 args。条件筛选返回 0 结果时不得自行改条件；如实保留空结果并继续其它证据步骤。非筛选查询可换一种不改变原意的自然问法、补其它金融 Skill，或最后用 anysearch 兜底。
...
{{
  "plan": [
    {{"goal": "获取今日涨停股票及可用于筛选的成交、换手、市值等指标", "target_skill": "financial-query",
     "args": {{"query": "今日A股涨停股票，按成交额、换手率、总市值列出", "limit": "20"}}}},
    {{"goal": "查询今日涨停股及其所属热点的近期新闻和催化", "target_skill": "news-search",
     "args": {{"query": "今日A股涨停股票 近期新闻 催化 风险", "days": "7", "limit": "20"}}}},
    {{"goal": "查询今日涨停股的近期重大公告和风险事件", "target_skill": "announcement-search",
     "args": {{"query": "今日A股涨停股票 近期重大公告 风险提示", "days": "30", "limit": "20"}}}},
    {{"goal": "补充候选股的机构观点和基本面风险", "target_skill": "report-search",
     "args": {{"query": "今日A股涨停股票 近期研报 业绩 风险", "days": "90", "limit": "20"}}}}
  ],
  "rationale": "买入判断需要同时核对涨停强度、催化、公告风险和基本面证据，不能只根据当日涨停下结论"
}}
