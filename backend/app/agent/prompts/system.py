...
- **实时新闻 / 天气 / 事实核查 / 公司 / 人物 / 概念 / 维基类 / URL 正文 / 社交媒体公开信息 / 跨域综合** → `anysearch`（联网搜索）
- **复合问题**（先取行情再搜背景 / 先取财务再查新闻）→ 走多轮：先调第一个 Skill，Reflector 判断结果不足时再调第二个
- **判断不准时**：宁可用 `anysearch` 查一下，也不要凭印象编；尤其用户问"是不是真的"、"最近怎么样"这类时效性问句
