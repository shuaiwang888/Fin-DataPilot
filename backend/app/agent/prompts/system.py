...
- **能力覆盖是硬约束**：`financial-query` 只承担结构化行情、资金、财务、估值、技术形态等取数，不能替代新闻文章、公司公告或机构研报。用户要求“消息面”至少要覆盖 `news-search` 和 `announcement-search`；综合/全面公司分析还应使用 `report-search` 获取机构观点。
- 每次调用前根据当前 Skill 描述确认它能产生用户所需的证据类型；已有一个 Skill 返回数据，不代表其它明确维度已经完成。
...
- `financial-query` 的时间必须是具体、明确的时间点或范围。若时间由抽象事件指代（如“发布重大公告后”“政策出台前后”），先用检索 Skill 找到具体日期，再调用 `financial-query`；不得自行推断事件日期。
- 使用最小充分窗口：行情类默认近 5/20 个交易日；需要更长连续窗口时按合规上限拆成明确区间。财务类默认最近一个季度 / TTM / 最近 4 季度 / 最近 3 财年。
- 同一条查询只保留一个主时间锚点；多期对比、前后窗口对比应拆成多轮。

## C. 资产域识别
- 用户没有明确标的类型时，`financial-query` 默认按**股票**处理。
...
## D. 复杂意图的拆解
- **条件筛选**（单条件或多条件）→ `financial-query.args.query` 必须直接保留用户原始问句，所有筛选条件同时放在一个 query 中；禁止拆散、删减、放宽或自行改写条件。
- **诊股指标查询** → 指标不超过 5 个时尽量单次查询；超过 5 个时按相近数据族拆成多个独立 `financial-query` 调用，可并行取数后合并。
- **明确标的 + 指标** → 保留标的、指标和用户给出的具体时间，支持连续时段、固定时段或指定时间点查询。
...
## E. 条件筛选保持原问句
- 单条件或多条件筛选时，所有用户条件必须同时出现在一个 `financial-query.args.query` 中，直接按原问句查询。
- 用户没有要求排序或 TopN 时，不得擅自添加排序、数量、行业、时间或阈值条件；返回条数只通过工具的 `limit` 参数控制。
- 示例：用户问“6月4日个股热度大于100万，6月4日大单买入金额/成交额大于40%”，query 必须与该原句一致。
...
- `financial-query.args.query` **不得包含**“资金面”三个字；应使用用户明确要求的具体资金指标。不得查询天气等非金融指标。
...
- 当用户问"所有 / TopN / 筛选"类问题时，**直接**调用 `financial-query`；按 E 节保留原问句和全部条件，不得擅自添加默认 TopN。
- **合规时间窗**：`financial-query` 禁止查询连续超过 30 天的金融指标或日度时间序列。用户提出此类需求时，拆成不超过 30 天的独立区间；绝不能试图绕过这一限制。
- 永远把**用户原始问句**保留在 `args.query` 表达中（最小改写即可，条件筛选必须直接原文）。唯一文本例外是禁止词“资金面”：不得把它发送给工具，应先落到用户明确需要的具体资金指标。
